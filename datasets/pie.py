...
class PIE(torch.utils.data.Dataset):
    def __init__(self, 
                data_dir,
                out_dir,
                dtype,
        ...
        self.filename = 'pie_{}_{}_{}_{}.csv'.format(dtype, str(input),\
        ...
        if(from_file):
            ...
            df['ped_attribute'] = df[['age', 'gender']].apply(lambda row: [row.age, row.gender], axis=1)
            df['ped_behavior'] = df[['action','gesture','look']].apply(lambda row: [row.action, row.gesture, row.look], axis=1)
    ...
    def _load_image_op(self, image_path_op):
        # print(image_path_op)
        optical_flow = cv2.imread(image_path_op, cv2.IMREAD_UNCHANGED)

           
            # Convert to PIL Image
        image_op= Image.fromarray(optical_flow).resize((self.image_resize[0], self.image_resize[1]))
