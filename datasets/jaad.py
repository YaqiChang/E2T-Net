...
class JAAD(torch.utils.data.Dataset):
        ...
        def generator():
            for index, item in enumerate(iterable, start=1):
                if index == 1 or index % step == 0 or index == total:
                    filled = int(round(20 * index / total))
                    bar = '#' * filled + '-' * (20 - filled)
                    sys.stdout.write(
                        f"\r{desc}: [{bar}] {index}/{total} ({100.0 * index / total:0.1f}%)"
                    )
                    sys.stdout.flush()
                    if index == total:
                        sys.stdout.write('\n')
    ...
    def _compute_centers_with_progress(self, df):
        if tqdm is not None:
            tqdm.pandas(desc='Computing centers', leave=False)
            return df.progress_apply(lambda row: utils.compute_center(row), axis=1)
